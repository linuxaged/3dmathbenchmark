...
static float ksVector3f_Dot( const ksVector3f * a,const ksVector3f * b)
{
    return a->x*b->x + a->y*b->y + a->z*b->z;
}

static void ksVector3f_Cross( ksVector3f * result, const ksVector3f * a,const ksVector3f * b)
{
...
//static void ksMatrix4x4f_LookAtRH( ksMatrix4x4f * camera, const ksVector3f& eye, const ksVector3f& fwd, const ksVector3f& up)
static ksMatrix4x4f ksMatrix4x4f_LookAtRH( const ksVector3f& eye, const ksVector3f& target, const ksVector3f& up)
{
    // FIXME: this fails when looking straight up, should probably at least assert

    // Forward : (eye - at).Normalized();
    ksVector3f z;
    ksVector3f_Sub( &z, &eye , &target);
    //ksVector3f_Sub( &z, &fwd, &eye );
    ksVector3f_Normalize( &z);

    // Right : up.Cross(z).Normalized();
    ksVector3f x;
    ksVector3f_Cross( &x, &up, &z);
    ksVector3f_Normalize(&x);

    ksVector3f y;
    ksVector3f_Cross( &y, &z, &x);

//    ksMatrix4x4f m{x.x,  x.y,  x.z,  -ksVector3f_Dot(&x, &eye),//-(x.Dot(eye)),
//                   y.x,  y.y,  y.z, -ksVector3f_Dot(&y, &eye), //-(y.Dot(eye)),
//                   z.x,  z.y,  z.z,  -ksVector3f_Dot(&z, &eye),  //-(z.Dot(eye)),
//                   0,    0,    0,    1 };
//    ksMatrix4x4f dstMat;
//    ksMatrix4x4f_Transpose( &dstMat, &m);

    ksMatrix4x4f result{ x.x, y.x, z.x, 0,
                         x.y, y.y, z.y, 0,
                         x.z, y.z, z.z, 0,
                         -ksVector3f_Dot(&x, &eye), -ksVector3f_Dot(&y, &eye), -ksVector3f_Dot(&z, &eye), 1 };
    return result;
}
